 ...
 */
router.get('/owner/summary', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const dueVaccinations = await prisma_1.prisma.vaccination.findMany({
            where: {
        ...
        const activeMedications = await prisma_1.prisma.medication.findMany({
            where: {
                petId: { in: petIds.length ? petIds : ['__none__'] },
 ...
 * Auth: ADMIN
 */
router.get('/admin/kpi', auth_1.requireAuth, (0, auth_1.requireRole)('ADMIN'), async (req, res) => {
    try {
        ...
        const [booked, completed, cancelled, total, uniqueOwners] = await Promise.all([
            prisma_1.prisma.appointment.count({ where: { ...whereRange, status: STATUS.BOOKED } }),
            prisma_1.prisma.appointment.count({ where: { ...whereRange, status: STATUS.COMPLETED } }),
            prisma_1.prisma.appointment.count({ where: { ...whereRange, status: STATUS.CANCELLED } }),
            prisma_1.prisma.appointment.count({ where: whereRange }),
        ...
        const byVet = await prisma_1.prisma.appointment.groupBy({
            by: ['vetId'],
            where: whereRange,
            _count: { vetId: true },
        ...
        const vetIds = byVet.map(v => v.vetId).filter(Boolean);
        const vets = vetIds.length
            ? await prisma_1.prisma.vet.findMany({ where: { id: { in: vetIds } }, select: { id: true, name: true } })
            : [];
