...
// ---------- Helpers ----------
async function assertPetOwnershipOrElevated(req, petId) {
    if (req.user.role === "ADMIN" || req.user.role === "VET")
        return;
    const owned = await prisma_1.prisma.pet.findFirst({
...
router.post("/vaccinations", auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const created = await prisma_1.prisma.vaccination.create({
            data: {
...
router.get("/vaccinations", auth_1.requireAuth, async (req, res) => {
    const petId = String(req.query.petId || "");
    if (!petId)
        return res.status(400).json({ ok: false, error: "petId is required" });
    try {
        await assertPetOwnershipOrElevated(req, petId);
        const list = await prisma_1.prisma.vaccination.findMany({
            where: { petId },
...
router.post("/medications", auth_1.requireAuth, async (req, res) => {
...
router.get("/medications", auth_1.requireAuth, async (req, res) => {
    const petId = String(req.query.petId || "");
    if (!petId)
        return res.status(400).json({ ok: false, error: "petId is required" });
    try {
        await assertPetOwnershipOrElevated(req, petId);
        const list = await prisma_1.prisma.medication.findMany({
            where: { petId },
...
router.get("/timeline/:petId", auth_1.requireAuth, async (req, res) => {
    const petId = req.params.petId;
    try {
        await assertPetOwnershipOrElevated(req, petId);
        const [vaccs, meds] = await Promise.all([
            prisma_1.prisma.vaccination.findMany({
                where: { petId },
                select: { id: true, vaccineName: true, givenDate: true, nextDueDate: true, notes: true },
                orderBy: { givenDate: "desc" },
            }),
            prisma_1.prisma.medication.findMany({
                where: { petId },
