...
import {
  CreateVetSchema, UpdateVetSchema,
...
// Update vet (ADMIN)
router.patch("/:vetId", requireAuth, requireRole("ADMIN"), async (req, res) => {
  const { vetId } = req.params;
  const parsed = UpdateVetSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ ok: false, error: parsed.error.issues });
...
// Archive vet (ADMIN)
router.delete("/:vetId", requireAuth, requireRole("ADMIN"), async (req, res) => {
  const { vetId } = req.params;
  try {
    const vet = await prisma.vet.update({
      where: { id: vetId },
...
router.get("/:vetId/availability", requireAuth, async (req, res) => {
  const { vetId } = req.params;
  try {
...
// Create a slot (ADMIN)
router.post("/:vetId/availability", requireAuth, requireRole("ADMIN"), async (req, res) => {
  const { vetId } = req.params;
  ...
  try {
    ...
    if (overlapping) return res.status(409).json({ ok: false, error: "Slot overlaps existing availability" });
...
// Update a slot (ADMIN)
router.patch("/:vetId/availability/:slotId", requireAuth, requireRole("ADMIN"), async (req, res) => {
  ...
  try {
    ...
    if (parsed.data.startAt || parsed.data.endAt) {
      const current = await prisma.availability.findUnique({ where: { id: slotId } });
      if (!current || current.vetId !== vetId) return res.status(404).json({ ok: false, error: "Slot not found" });
      ...
      if (overlapping) return res.status(409).json({ ok: false, error: "Updated slot overlaps another availability" });
    }
...
// Delete a slot (ADMIN)
router.delete("/:vetId/availability/:slotId", requireAuth, requireRole("ADMIN"), async (req, res) => {
  const { vetId, slotId } = req.params;
  try {
    const current = await prisma.availability.findUnique({ where: { id: slotId } });
    if (!current || current.vetId !== vetId) return res.status(404).json({ ok: false, error: "Slot not found" });
